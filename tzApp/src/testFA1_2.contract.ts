...
export type ApproveParams = TRecord<
    {
        spender: TAddress;
        value: TNat;
    },
...
export type TransferParams = TRecord<
    {
        from: TAddress;
        to: TAddress;
        value: TNat;
...
class Storage {
    ...
    storage: TStorage = {
        config: {
            admin: 'tz1bLBkLUWcnQbCC8pCcjtwZvekLUwPPEsRq', ////NE Deloyment
            paused: false,
        },
        balances: [],
        totalSupply: 0,
...
class Metadata extends OffChainViews {
    @MetadataBuilder
    metadata = {
        name: 'CrossChain loan FA1.2 Token Template',
        description: 'test Token  for an FA1.2 Contract',
        authors: ['dee'],
        homepage: 'https://newearthart.tech',
...
@Contract
export class NETest_FA1_2 extends Helpers {
    @EntryPoint
    ...
    @EntryPoint
    giveMe(): void {
        //this.mint({address: Sp.sender, value: 100000});
        if (!this.addressExists(Sp.sender)) {
            const value: TBalance = { approvals: [], balance: 0 };
            this.storage.balances.set(Sp.sender, value);
        }
        const pVal = 10;
        this.storage.balances.get(Sp.sender).balance += pVal;
        this.storage.totalSupply += pVal;
...
Dev.test({ name: 'NETest_FA1_2' }, () => {
    const c1 = Scenario.originate(new NETest_FA1_2());
    Scenario.transfer(c1.mint({ address: 'KT18amZmM5W7qDWVt2pH6uj7sCEd3kbzLrHT', value: 1000000 }), {
        sender: 'tz1bLBkLUWcnQbCC8pCcjtwZvekLUwPPEsRq', //NE Deloyment
    });
});

Dev.compileContract('version_1', new NETest_FA1_2());
